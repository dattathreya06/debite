        ...
        >
          ...
          <div className="relative h-full flex flex-col justify-end p-6 z-10">
            {category && (
              <span 
                ref={categoryRef}
                className="mb-4 px-3 py-1 text-sm font-medium text-royal_blue_traditional-900 bg-gold dark:bg-gray-800/90 rounded-full transform-gpu self-start"
              >
                {category}
              </span>
            )}
            <h3 ref={titleRef} className="text-2xl font-bold mb-3 leading-tight">{title}</h3>
            <p ref={descRef} className="text-sm text-gray-200 line-clamp-3">{description}</p>
            
            <span 
              ref={linkRef}
              className="mt-4 text-gold-400 text-sm font-medium inline-flex items-center gap-1"
            >
              Read more
              <svg 
                className="w-4 h-4" 
  ...
  return (
    <Link href={href} className="block">
      <div 
        ...
        `}
      >
        <div ref={imageRef} className="card-image relative overflow-hidden h-1/2">
          <Image 
            src={imageUrl}
            alt={title}
            fill
        ...
        <div className="p-6">
          <h3 ref={titleRef} className="text-xl font-bold mb-3 leading-tight">{title}</h3>
          <p ref={descRef} className="text-sm text-gray-600 dark:text-gray-300 line-clamp-3">{description}</p>
        </div>
        <div className="absolute bottom-4 right-4 transform-gpu">
          <span 
            ref={linkRef}
            className="text-royal_blue_traditional-400 dark:text-royal_blue_traditional-300 text-sm font-medium inline-flex items-center gap-1"
          >
            Read more
            <svg 
              className="w-4 h-4" 
        ...
        <div className="absolute bottom-0 left-0 right-0 overflow-hidden">
          <div 
            ref={borderRef}
            className="w-full h-[2px] bg-royal_blue_traditional-400 origin-left"
            style={{ transform: 'scaleX(0)' }}
          />
        </div>
      </div>
